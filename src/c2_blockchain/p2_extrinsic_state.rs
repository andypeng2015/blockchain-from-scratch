...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        if self.height != 0 || self.parent != 0 || self.state != 0 || self.extrinsic != 0 {
            return false;
        }
        if chain.len() == 0 {
            return true;
        ...
        let mut parent = self;
        for h in chain.iter() {
            if h.state != parent.state + h.extrinsic {
                return false;
            }
            if h.height != parent.height + 1 {
                return false;
            }
            if h.parent != hash(parent) {
                return false;
            }
            parent = h;
        }
...
fn build_valid_chain(n: u64) -> Vec<Header> {
    ...
    for i in 1..n {
        ...
        chain[i] = Header {
            ...
            extrinsic: e,
            state: parent.state + e,
            consensus_digest: (),
        };
        parent = chain[i].clone();
    }
    chain
}
...
fn build_forked_chain() -> (Vec<Header>, Vec<Header>) {
    let g = build_valid_chain(5);
    let mut c1 = g.clone();
    let mut c2 = g.clone();
    let l1 = c1[c1.len() - 1].child(6);
    c1.push(l1);
    let l2 = c2[c2.len() - 1].child(9);
    c2.push(l2);
    (c1, c2)
